...
--
-- Host: localhost
-- Generation Time: Jul 24, 2024 at 01:48 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `check_and_update_pengiriman` (IN `p_id_pengiriman` INT, IN `p_status_baru` VARCHAR(255))   BEGIN
    DECLARE current_status VARCHAR(255);
    SELECT status_pengiriman INTO current_status
    FROM pengiriman
    WHERE id_pengiriman = p_id_pengiriman;

    IF current_status IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Pengiriman tidak ditemukan dengan ID yang diberikan.';
    ELSE
        CASE
            WHEN current_status = 'dikirim' THEN
                SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Status pengiriman tidak dapat diubah dari "dikirim".';
            WHEN current_status = 'diterima' THEN
                SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Status pengiriman sudah diterima dan tidak dapat diubah.';
            ELSE
                UPDATE pengiriman
                SET status_pengiriman = p_status_baru
                WHERE id_pengiriman = p_id_pengiriman;

                SELECT CONCAT('Status pengiriman dengan ID ', p_id_pengiriman, ' telah diperbarui menjadi ', p_status_baru) AS Message;
        END CASE;
    END IF;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `print_all_pengiriman` ()   BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `total_barang_in_gudang` () RETURNS INT(11)  BEGIN
    DECLARE total INT;
...
INSERT INTO `barang` (`id_barang`, `id_pengiriman`, `nama`, `deskripsi`, `stok`) VALUES
('EA001', 1, 'carrier', 'carrier ini berkapasitas 60L', 100),
('EA002', 2, 'stand_laptop', 'stand laptop ini sangat kokoh dan tahan lama', 150),
('EA003', 3, 'POCO F5', 'HP ini dilengkapi dengan chipeset G99', 200),
('EA004', 4, 'key FANTECH', 'keyboard mechanical dengan menggunakan switch biru', 250),
('EA005', 5, 'LOQ', 'laptop ini menggunakan processor INTEL 5', 300),
('EA006', 1, 'monitor', 'Monitor 24 inci dengan resolusi 1080p', 10),
('EA007', 2, 'printer', 'Printer laser hitam putih', 15),
('EA008', 3, 'mouse', 'Mouse gaming dengan DPI tinggi', 20),
('EA009', 4, 'speaker', 'Speaker bluetooth dengan bass kuat', 50),
('EA010', 5, 'headphone', 'Headphone noise-cancelling', 30);
...
(11, 12, 'dikirim', 'umbulharjo, jogja', '2024-07-24 08:23:44');
...
(12, 2, 1, '2024-07-06 00:00:00', 'SP1235', 'dikirim', 'umbulharjo, jogja');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_inside`  AS SELECT `pelanggan`.`id_pelanggan` AS `id_pelanggan`, `pelanggan`.`nama` AS `nama` FROM `pelanggan` WHERE `pelanggan`.`nama` like 'A%'WITH CASCADED CHECK OPTION  ;
